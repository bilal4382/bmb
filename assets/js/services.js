...
/* ═══════════════════════════════════════════════════════
   NOISE CANVAS
═══════════════════════════════════════════════════════ */
function initSvcNoise() {
  const c = document.getElementById('svcHeroNoise');
  if (!c) return;
...
═══════════════════════════════════════════════════════ */
function initSvcHeroBg() {
  const c = document.getElementById('svcHeroBg');
  if (!c) return;
  const ctx = c.getContext('2d');
  let W, H, t = 0, stars = [];
...
/* ═══════════════════════════════════════════════════════
   HERO ENTRANCE
═══════════════════════════════════════════════════════ */
function revealSvcHero() {
  const words = document.querySelectorAll('.h1w');
  const sub   = document.getElementById('svcSub');
  const index = document.getElementById('svcIndex');
  const stats = document.getElementById('svcStats');
  ...
  tl
    ...
    .to(index, { opacity:1, x:0, duration:.65 }, '-=.5')
  ...
  initServiceSections();
  initDoctorsSection();
  initServiceNavHighlight();
  initFooterBg();
}

/* ═══════════════════════════════════════════════════════
   SERVICE SECTIONS — scroll-triggered entrance animations
═══════════════════════════════════════════════════════ */
function initServiceSections() {
  document.querySelectorAll('.sp-section').forEach(section => {
    const dir      = section.dataset.dir;
    const visual   = section.querySelector('.sp-visual');
    const content  = section.querySelector('.sp-content');
    const bigNum   = section.querySelector('.sp-big-num');
    const iconWrap = section.querySelector('.sp-icon-wrap');
    const eyebrow  = section.querySelector('.sec-eyebrow');
    const h2a      = section.querySelector('.sp-h2a');
    const h2b      = section.querySelector('.sp-h2b');
    const lead     = section.querySelector('.sp-lead');
    const body     = section.querySelector('.sp-body');
    const dels     = section.querySelectorAll('.sp-del');
    const tags     = section.querySelectorAll('.sp-tags span');
    const cta      = section.querySelector('.sp-cta');
    const steps    = section.querySelectorAll('.sp-step');

    const xV = dir === 'left' ? -90 : 90;
    const xC = dir === 'left' ?  90 : -90;

    /* Set initial states */
    gsap.set(visual,   { x: xV, opacity: 0 });
    gsap.set(content,  { x: xC, opacity: 0 });
    gsap.set(bigNum,   { opacity: 0, scale: 1.25 });
    gsap.set(iconWrap, { scale: .75, opacity: 0, rotation: -12 });
    gsap.set(steps,    { x: -18, opacity: 0 });
    gsap.set(eyebrow,  { y: 18, opacity: 0 });
    gsap.set(h2a,      { y: 50, opacity: 0 });
    gsap.set(h2b,      { y: 60, opacity: 0 });
    gsap.set(lead,     { y: 28, opacity: 0 });
    gsap.set(body,     { y: 22, opacity: 0 });
    gsap.set(dels,     { x: -16, opacity: 0 });
    gsap.set(tags,     { scale: .8, opacity: 0 });
    if (cta) gsap.set(cta, { y: 14, opacity: 0 });
    ...
    ScrollTrigger.create({
      ...
      onEnter() {
        const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });
        tl
          /* Visual flies in from its side */
          .to(visual,   { x: 0, opacity: 1, duration: .9 })
          /* Big ghost number counter-scales */
          .to(bigNum,   { opacity: 1, scale: 1, duration: 1.1, ease: 'power2.out' }, '-=.85')
          /* Icon wrap springs in */
          .to(iconWrap, { scale: 1, opacity: 1, rotation: 0, duration: .8, ease: 'back.out(1.5)' }, '-=.72')
          /* Steps stagger in */
          .to(steps,    { x: 0, opacity: 1, duration: .5, stagger: .09 }, '-=.45')
          /* Content enters from opposite side */
          .to(content,  { x: 0, opacity: 1, duration: .85 }, '-=.65')
          .to(eyebrow,  { y: 0, opacity: 1, duration: .5  }, '-=.7')
          .to(h2a,      { y: 0, opacity: 1, duration: .6  }, '-=.55')
          .to(h2b,      { y: 0, opacity: 1, duration: .65 }, '-=.48')
          .to(lead,     { y: 0, opacity: 1, duration: .5  }, '-=.38')
          .to(body,     { y: 0, opacity: 1, duration: .5  }, '-=.38')
          /* Deliverables stagger from left */
          .to(dels,     { x: 0, opacity: 1, duration: .42, stagger: .065 }, '-=.3')
          /* Tags pop with scale */
          .to(tags,     { scale: 1, opacity: 1, duration: .38, stagger: .05, ease: 'back.out(1.7)' }, '-=.22')
          /* CTA last */
          .to(cta,      { y: 0, opacity: 1, duration: .4  }, '-=.18');
      }
    });
  });
}
...
/* ═══════════════════════════════════════════════════════
   DOCTORS TEASER — entrance animation
═══════════════════════════════════════════════════════ */
function initDoctorsSection() {
  const section = document.querySelector('.sp-doctors');
  if (!section) return;

  const visual   = document.getElementById('spDocVisual');
  const content  = document.getElementById('spDocContent');
  const rings    = section.querySelectorAll('.sp-doc-ring');
  const orbits   = section.querySelectorAll('.sp-doc-orbit');
  const cross    = section.querySelector('.sp-doc-cross');
  const badge    = section.querySelector('.sp-doc-badge');
  const eyebrow  = section.querySelector('.sec-eyebrow');
  const h2a      = section.querySelector('.sp-doc-h2a');
  const h2b      = section.querySelector('.sp-doc-h2b');
  const lead     = section.querySelector('.sp-doc-lead');
  const body     = section.querySelector('.sp-doc-body');
  const feats    = section.querySelectorAll('.sp-doc-feat');
  const actions  = section.querySelector('.sp-doc-actions');

  gsap.set(visual,  { x: -100, opacity: 0 });
  gsap.set(content, { x:  100, opacity: 0 });
  gsap.set(cross,   { scale: 0, opacity: 0, rotation: -90 });
  gsap.set(rings,   { scale: 0, opacity: 0 });
  gsap.set(orbits,  { scale: 0, opacity: 0 });
  gsap.set(badge,   { y: -20, opacity: 0 });
  gsap.set(eyebrow, { y: 18, opacity: 0 });
  gsap.set(h2a,     { y: 50, opacity: 0 });
  gsap.set(h2b,     { y: 60, opacity: 0 });
  gsap.set(lead,    { y: 24, opacity: 0 });
  gsap.set(body,    { y: 20, opacity: 0 });
  gsap.set(feats,   { x: -20, opacity: 0 });
  ...
  ScrollTrigger.create({
    ...
    onEnter() {
      const tl = gsap.timeline({ defaults: { ease: 'power3.out' } });
      tl
        /* Both sides enter simultaneously */
        .to(visual,   { x: 0, opacity: 1, duration: 1.0 })
        .to(content,  { x: 0, opacity: 1, duration: 1.0 }, '-=1.0')
        /* Cross spins into existence */
        .to(cross,    { scale: 1, opacity: 1, rotation: 0, duration: .8, ease: 'back.out(1.8)' }, '-=.7')
        /* Rings radiate outward */
        .to(rings[0], { scale: 1, opacity: 1, duration: .65, ease: 'expo.out' }, '-=.5')
        .to(rings[1], { scale: 1, opacity: 1, duration: .8,  ease: 'expo.out' }, '-=.55')
        .to(rings[2], { scale: 1, opacity: 1, duration: 1.0, ease: 'expo.out' }, '-=.6')
        /* Orbit icons pop in */
        .to(orbits,   { scale: 1, opacity: 1, duration: .5, stagger: .1, ease: 'back.out(2)' }, '-=.4')
        /* Badge drops */
        .to(badge,    { y: 0, opacity: 1, duration: .45, ease: 'back.out(1.6)' }, '-=.3')
        /* Content */
        .to(eyebrow,  { y: 0, opacity: 1, duration: .5  }, '-=.8')
        .to(h2a,      { y: 0, opacity: 1, duration: .6  }, '-=.55')
        .to(h2b,      { y: 0, opacity: 1, duration: .65 }, '-=.48')
        .to(lead,     { y: 0, opacity: 1, duration: .5  }, '-=.38')
        .to(body,     { y: 0, opacity: 1, duration: .5  }, '-=.38')
        .to(feats,    { x: 0, opacity: 1, duration: .42, stagger: .08 }, '-=.25')
        .to(actions,  { y: 0, opacity: 1, duration: .4  }, '-=.15');
    }
  });
}

/* ═══════════════════════════════════════════════════════
   SERVICE INDEX HIGHLIGHT — active section tracking
═══════════════════════════════════════════════════════ */
function initServiceNavHighlight() {
  const links    = document.querySelectorAll('.svc-idx-link');
  const sections = document.querySelectorAll('.sp-section, .sp-doctors');
  ...
  const io = new IntersectionObserver(entries => {
    entries.forEach(en => {
      ...
      links.forEach(l => l.classList.remove('phi-active'));
      const active = document.querySelector(`.svc-idx-link[href="#${id}"]`);
      if (active) active.classList.add('phi-active');
    });
